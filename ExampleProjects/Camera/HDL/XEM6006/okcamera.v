//------------------------------------------------------------------------
// okcamera.v - Top-level EVB1006 camera HDL for the XEM6006
//
// Clocks:
//    CLK_SYS    - 48 MHz single-ended input clock
...
clocks # (
		...
		.C_INCLK_PERIOD          (20833),     // System clock is 48 MHz
		.C_DIVCLK_DIVIDE         (2),         // Divide to 24 MHz
		.C_CLKFBOUT_MULT         (26),        // VCO frequency is 624 MHz
		.C_CLKDDR0_DIVIDE        (1),         // DDR @ 620 MHz (0-phase)
		.C_CLKDDR1_DIVIDE        (1),         // DDR @ 620 MHz (180-phase)
		.C_CLKSYS_DIVIDE         (6),         // Logic clock is 104 MHz
		.C_CLKMCB_DIVIDE         (8),         // MCB @ 78 MHz
		.C_CLKPIXEXT_DIVIDE      (26),        // Image sensor EXTCLK output is 24 MHz
...
wire [17*10-1:0]  ok2x;
okWireOR # (.N(10)) wireOR (.ok2(ok2), .ok2s(ok2x));
...
okTriggerIn  ti40b (.ok1(ok1),                           .ep_addr(8'h40), .ep_clk(clk0),     .ep_trigger(ti40_mig));
...
okWireOut    wo3e  (.ok1(ok1), .ok2(ok2x[ 8*17 +: 17 ]), .ep_addr(8'h3e), .ep_datain(CAPABILITY));
okWireOut    wo3f  (.ok1(ok1), .ok2(ok2x[ 9*17 +: 17 ]), .ep_addr(8'h3f), .ep_datain(VERSION));
