...
export abstract class FrontPanelCamera implements ICamera {
    ...
    public async Initialize(): Promise<number> {
        ...
        this._HDLVersion = await this._FrontPanel.getWireOutValue(0x3f);
    ...
    public async SetImageBufferDepth(depth: FrameCount): Promise<void> {
        ...
        await this._FrontPanel.setWireInValue(0x05, 0x400 | frameCount, 0x7ff);
    ...
    public GetCapabilities(): Promise<number> {
    ...
    public async GetMissedFrameCount(): Promise<FrameCount> {
        ...
        return (await this._FrontPanel.getWireOutValue(0x23)) & 0xff;
    }

    public async GetBufferedFrameCount(): Promise<FrameCount> {
        await this._FrontPanel.updateWireOuts();

        return await this._FrontPanel.getWireOutValue(0x24);
    }

    public async LogicReset(): Promise<void> {
        await this._FrontPanel.setWireInValue(0x00, 0x0008, 0x0008);
        await this._FrontPanel.updateWireIns();
        
        // Add a delay of 50 milliseconds
        await new Promise(resolve => setTimeout(resolve, 50));
        
        await this._FrontPanel.setWireInValue(0x00, 0x0000, 0x0008);
        await this._FrontPanel.updateWireIns();
    }

    public async EnablePingPong(enable: boolean): Promise<void> {
        if (enable) {
            await this._FrontPanel.setWireInValue(0x00, 1 << 4, 1 << 4);
            await this._FrontPanel.updateWireIns();
        } else {
            await this._FrontPanel.setWireInValue(0x00, 0 << 4, 1 << 4);
            await this._FrontPanel.updateWireIns();
        }

        // Reset things
        await this._FrontPanel.setWireInValue(0x00, 1 << 3, 1 << 3);
        await this._FrontPanel.updateWireIns();
        
        // Add a delay of 50 milliseconds
        await new Promise(resolve => setTimeout(resolve, 50));
        
        await this._FrontPanel.setWireInValue(0x00, 0 << 3, 1 << 3);
        await this._FrontPanel.updateWireIns();
    }
    ...
    public async BufferedCaptureV1(ulLen: number): Promise<FrameCaptureResult> {
        await this._FrontPanel.updateWireOuts();
        let done: boolean = ((await this._FrontPanel.getWireOutValue(0x23)) & 0x0300) > 0;

        for (let i = 1; i < 100 && !done; i++) {
            await sleep(2);
            await this._FrontPanel.updateWireOuts();

            done = ((await this._FrontPanel.getWireOutValue(0x23)) & 0x0300) > 0;
        }
        ...
        const frameBufferId: number = await this._FrontPanel.getWireOutValue(0x23);

        if ((frameBufferId & 0x0100) > 0) {
            // Frame ready(buffer A)
            await this._FrontPanel.setWireInValue(0x04, 0x0000, 0xffffffff);
            await this._FrontPanel.setWireInValue(0x05, 0x0000, 0xffffffff);
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.activateTriggerIn(0x40, 1);

            // Readout start trigger
            const data: ArrayBuffer = await this._FrontPanel.readFromPipeOut(0xa0, ulLen);
        ...
        // console.log(uint32Array);
            ...
            await this._FrontPanel.setWireInValue(0x04, 0x0000, 0xffffffff);
            await this._FrontPanel.setWireInValue(0x05, 0x0080, 0xffffffff);
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.activateTriggerIn(0x40, 1);
    ...
    public async SingleCaptureV1(ulLen: number): Promise<FrameCaptureResult> {
        ...
        await this._FrontPanel.setWireInValue(0x00, 0 << 4, 1 << 4);
        //--Set data length
        await this._FrontPanel.setWireInValue(0x02, ulLen & 0xffff, DEFAULT_MASK);
        await this._FrontPanel.setWireInValue(0x03, ulLen >> 16, DEFAULT_MASK);
        //--Readout address = 0x00000000
        await this._FrontPanel.setWireInValue(0x04, 0x0000, DEFAULT_MASK);
        await this._FrontPanel.setWireInValue(0x05, 0x0000, DEFAULT_MASK);
        await this._FrontPanel.updateWireIns();
        ...
        //
        await this._FrontPanel.updateTriggerOuts();
        let done: boolean = await this._FrontPanel.isTriggered(0x60, 1 << 0); // Frame done trigger

        for (let index = 1; index < 500 && !done; index++) {
            await sleep(1);
            await this._FrontPanel.updateTriggerOuts();

            done = await this._FrontPanel.isTriggered(0x60, 1 << 0); // Frame done trigger
        }
    ...
    public async BufferedCaptureV2(ulLen: number): Promise<FrameCaptureResult> {
        await this._FrontPanel.updateWireOuts();
        let done: boolean = ((await this._FrontPanel.getWireOutValue(0x23)) & 0x0100) !== 0; // Frame avail ?

        for (let index = 0; index < 100 && !done; index++) {
            await sleep(2);
            await this._FrontPanel.updateWireOuts();

            done = ((await this._FrontPanel.getWireOutValue(0x23)) & 0x0100) !== 0; // Frame avail ?
        }
    ...
    public async readFromBuffer(length:number): Promise<ArrayBuffer> {
        try {
          const hist = await this._FrontPanel.readFromPipeOut(0xA1, length);
    ...
    protected async AssertResets(): Promise<void> {
        ...
        // + Pixel Clock DCM
        // + Logic
        await this._FrontPanel.setWireInValue(0x00, 0x000f, 0x000f);
        await this._FrontPanel.updateWireIns();
        await sleep(1);
        await this._FrontPanel.setWireInValue(0x00, 0x0000, 0x0001); // Release system PLL RESET
        await this._FrontPanel.updateWireIns();
        await sleep(1);
        await this._FrontPanel.setWireInValue(0x00, 0x0000, 0x0002); // Release image sensor RESET
        await this._FrontPanel.updateWireIns();
        await sleep(1);
        await this._FrontPanel.setWireInValue(0x00, 0x0000, 0x0008); // Release logic RESET
        await this._FrontPanel.updateWireIns();
        await sleep(10);
    }
    ...
    protected async ReleaseResets(): Promise<void> {
        //Release PIXCLK DCM RESET
        await sleep(10);
        await this._FrontPanel.setWireInValue(0x00, 0x0000, 0x0004);
        await this._FrontPanel.setWireInValue(0x00, 0x0010, 0x0010);
        await this._FrontPanel.updateWireIns();
    }
