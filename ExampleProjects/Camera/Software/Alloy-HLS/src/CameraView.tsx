...
import DispHist from "./Hist_View";
...
class CameraView extends Component<CameraViewProps, CameraViewState> {
    ...
    render() {
        ...
        return (
            <div className="okCameraView">
                <div className="okCameraControlPanel">
                    ...
                    />
                        <NumberEntry
                            ...
                            numeralSystem={NumeralSystem.Decimal}
                            minimumValue={BigInt(0)}
                            maximumValue={BigInt(255)}
                            value={BigInt(this.state.rgain)}
                            onValueChange={this.RGainChanged}
                        />
                        <NumberEntry
                            label={{
                                text: "GGAIN",
                            ...
                            minimumValue={BigInt(0)}
                            maximumValue={BigInt(255)}
                            value={BigInt(this.state.ggain)}
                            onValueChange={this.GGainChanged}
                        />
                        <NumberEntry
                            label={{
                                text: "BGAIN",
                            ...
                            minimumValue={BigInt(0)}
                            maximumValue={BigInt(255)}
                            value={BigInt(this.state.bgain)}
                            onValueChange={this.BGainChanged}
                        />
                    <RangeSlider
                        label={{ text: "Black Level Correction" }}
                        minimumValue={0}
                        maximumValue={255}
                        value={(this.state.blc)}
                        onValueChange={this.BLCChanged}
                    />
                    <RangeSlider
                        label={{ text: "Auto White Balance" }}
                        minimumValue={0}
                        maximumValue={255}
                        value={(this.state.awb)}
                        onValueChange={this.AWBchanged}
                    />
                    <SelectEntry.Root
                        label={{
                            text: "Display Mode",
                ...
                {/* <div className="okCameraControlVerticalSeparator" /> */}
                {/* <HistogramView
                    data={this._Camera.readFromBuffer(256)}
                    width={200}
                    height={100}
                /> */}
                {/* <div className="okCameraControlVerticalSeparator" /> */}
                ...
                />
                <DispHist workQueue={this._WorkQueue} />
            </div>
        );
    // }
    }
    ...
    private async SetCameraState(
        ...
        await this._WorkQueue.Post(async (): Promise<void> => {
            ...
            await this._FrontPanel.setWireInValue(0x06, rgain, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.setWireInValue(0x07, ggain, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.setWireInValue(0x08, bgain, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.setWireInValue(0x09, blc, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
            await this._FrontPanel.setWireInValue(0x0A, awb, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
            ...
            await this._FrontPanel.setWireInValue(0x0B, skip, 0xffff); // Address Words
            await this._FrontPanel.updateWireIns();
    ...
    private async ContinuousFrameLoop(): Promise<void> {
        await this._WorkQueue.Post(async () => {
            ...
            // const hist: ArrayBuffer = await this._Camera.readFromBuffer(256);
            // console.log(hist);
    ...
    private RGainChanged = (value: bigint) => {
        console.log("Selected RGain Changed: " + value);
        ...
        this.setState({ rgain: Number(value) });
    };
    private GGainChanged = (value: bigint) => {
        console.log("Selected GGain Changed: " + value);
        ...
        this.setState({ ggain: Number(value) });
    };
    
    private BGainChanged = (value: bigint) => {
        console.log("Selected BGain Changed: " + value);
        ...
        this.setState({ bgain: Number(value) });
    };

    private BLCChanged = (value: number) => {
        console.log("Selected BLC Changed: " + value);
        ...
        this.setState({ blc: Number(value) });
    };

    private AWBchanged = (value: number) => {
        console.log("Selected AWB Thresh Changed: " + value);
        ...
        this.setState({ awb: Number(value) });
    };
    /**
