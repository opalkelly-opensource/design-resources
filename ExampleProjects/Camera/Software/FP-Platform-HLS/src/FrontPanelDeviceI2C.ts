...
export class FrontPanelDeviceI2C {
    ...
    public async Read16(address: number): Promise<number> {
        ...
        this._FrontPanel.setWireInValue(0x01, 0x0084, 0x00ff); // Address Words
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Starts
        this._FrontPanel.setWireInValue(0x01, 0x0004, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Stops
        this._FrontPanel.setWireInValue(0x01, 0x0000, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Payload Length(Bytes)
        this._FrontPanel.setWireInValue(0x01, 0x0002, 0x00ff); // Data Words
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Device Address(write)
        this._FrontPanel.setWireInValue(0x01, this._DeviceAddress, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Register Address(high)
        this._FrontPanel.setWireInValue(0x01, address >> 8, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Register Address(low)
        this._FrontPanel.setWireInValue(0x01, address, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Device Address(read)
        this._FrontPanel.setWireInValue(0x01, this._DeviceAddress + 1, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        ...
        } while (this._FrontPanel.isTriggered(0x61, 0x0001));

        await this._FrontPanel.activateTriggerIn(0x42, 1);
        // Read result 0
        await this._FrontPanel.updateWireOuts();

        u16Data = ((this._FrontPanel.getWireOutValue(0x22)) & 0xff) << 8;

        // Read result 1
        await this._FrontPanel.activateTriggerIn(0x42, 3);
        await this._FrontPanel.updateWireOuts();

        u16Data = u16Data | ((this._FrontPanel.getWireOutValue(0x22)) & 0xff);
    ...
    public async Write16(address: number, data: number): Promise<void> {
        await this._FrontPanel.activateTriggerIn(0x42, 1);
        // Preamble Length(Bytes)
        this._FrontPanel.setWireInValue(0x01, 0x0003, 0x00ff); // Address Words
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Starts
        this._FrontPanel.setWireInValue(0x01, 0x0000, 0x00ff);
        ...
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Data 0 MSB
        this._FrontPanel.setWireInValue(0x01, data >> 8, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
        // Data 1 LSB
        this._FrontPanel.setWireInValue(0x01, data, 0x00ff);
        await this._FrontPanel.updateWireIns();
        await this._FrontPanel.activateTriggerIn(0x42, 2);
